"""
generate_adjust_prompt.py
--------------------------------
Generates adjust_prompt.json (short SAM3 prompt lists) from contexts_class.json,
using llm

Input : contexts_class.json  {class: {definition, positive, negative, confusable_classes,
                                       anchor_prompts?, generation_notes?}}
Output: adjust_prompt.json   {class: [prompt1, prompt2, ...]}
        adjust_prompt_meta.json  (raw response + config, for paper reproducibility)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate adjust_prompt.json from contexts_class.json via Gemini."
    )
    parser.add_argument("--input", default=str(Path(__file__).resolve().parent.parent / "configs" / "contexts_class_voc.json"))
    parser.add_argument("--output", default=str(Path(__file__).resolve().parent.parent / "configs" / "adjust_prompt_voc.json"))
