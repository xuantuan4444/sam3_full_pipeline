...
voc_common.py
=============
Shared library for every step of the Pascal VOC 2012 pipeline.

Section A (DATASET SPEC) is the ONLY place that knows anything specific to Pascal VOC 2012:
...
run_pipeline_voc.py can import this module for its preflight/plan without a GPU stack.
"""
...
# ============================================================================
# A. DATASET SPEC -- Pascal VOC 2012
# ============================================================================
DATASET = "voc"
DATASET_TITLE = "Pascal VOC 2012"

ROOT = Path(__file__).resolve().parent
DATA_DIR = ROOT / "data" / "VOC2012"
IMAGES_DIR = DATA_DIR / "JPEGImages"
GT_DIR = DATA_DIR / "SegmentationClass"             # palette PNG: 0 = background, 1..20, 255 = void
TRAIN_LIST = DATA_DIR / "ImageSets" / "Segmentation" / "train.txt"   # 1464 ids
VAL_LIST = DATA_DIR / "ImageSets" / "Segmentation" / "val.txt"       # 1449 ids
REQUIRED_DATA_PATHS = [IMAGES_DIR, GT_DIR, TRAIN_LIST, VAL_LIST]

CONFIG_DIR = ROOT / "configs"
TARGET_CLASSES_JSON = CONFIG_DIR / "target_classes.json"
ADJUST_PROMPT_JSON = CONFIG_DIR / "adjust_prompt_voc.json"
...
# Evaluation label space: 0 = background, 1..20 = class, 255 = void (ignored). Pixels with no
# confident SAM3 detection are background (0). mIoU / mDice are averaged over the 20 object
# classes (background excluded), as in the original VOC notebooks.
EVAL_NUM_CLASSES = 21
EVAL_CLASS_OFFSET = 1
EVAL_NO_PREDICTION = 0
EVAL_IGNORE = 255
EVAL_METRIC_INDICES = list(range(1, 21))
EVAL_EXTRA_NAMES = {0: "background"}

# Training label space (get_coarse / train): raw VOC value, i+1 for the i-th class; 255 = void.
...
TRAIN_PARAMS = {
    ...
    "tversky": {
        "sofa": (0.50, 0.50),
        "chair": (0.45, 0.55),
        "diningtable": (0.45, 0.55),
        "tvmonitor": (0.40, 0.60),
        "pottedplant": (0.35, 0.65),
        "bicycle": (0.35, 0.65),
    },
    ...
    "ce_boost": {"bicycle": 1.5},
}


def list_ids(subset):
    path = {"train": TRAIN_LIST, "val": VAL_LIST}[subset]
...
def load_gt_train_raw(img_id, subset):
    """Raw palette indices: 0 = background, 1..20 = class, 255 = void."""
...
def load_gt_eval(img_id, subset):
    """Already in the eval label space (0 = background, 1..20, 255 = void)."""
    return np.array(Image.open(gt_path(img_id, subset)), dtype=np.uint8)
