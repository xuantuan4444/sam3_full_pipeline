#!/usr/bin/env python3
"""
run_pipeline_voc.py -- run the whole VOC pipeline in one go.

Per arm (nollm, llm), in this order:
    1. sam3_baseline_voc.py   SAM3 on val once -> baseline metrics + val_cache
    2. get_coarse_voc.py      SAM3 coarse masks on train (+ split.json if missing)
    3. train_unetaspp_voc.py
    4. train_unetasppdinov2_voc.py
    5. val_unetaspp_voc.py        (reads val_cache, no SAM3)
    6. val_unetasppdinov2_voc.py  (reads val_cache, no SAM3)
...
prompts and settings (fingerprint in manifest.json). Interrupted? Re-run the same command.
    ...
    python run_pipeline_voc.py --skip-nollm              # LLM arm only
    python run_pipeline_voc.py --skip-nollm --dry-run    # preflight + plan, runs nothing
    python run_pipeline_voc.py --skip-nollm --limit 5    # smoke test -> outputs_smoke/
"""
