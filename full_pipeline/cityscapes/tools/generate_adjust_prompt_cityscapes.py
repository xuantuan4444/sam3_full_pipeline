"""
generate_adjust_prompt_city_v3.py
--------------------------------
Generates adjust_prompt.json (short SAM3/CLIP prompt lists) from contexts_class.json,
using an LLM.

This is v3 of the generic open-vocabulary ("voc" = vocabulary, not Pascal VOC) prompt
generation pipeline, tightened for the Cityscapes class taxonomy after evaluation
showed that some classes regress when their bare class term is dropped in favor of
narrower sub-types, and that material/texture-adjective decompositions and thin
sub-part prompts also hurt recall. The fixes below are dataset-agnostic constraints
applied generically through the LLM instructions -- no run-specific metrics are
hard-coded here, since this file only drives LLM-based prompt generation, not
evaluation.

Changes vs. v2:
    1. SYSTEM_INSTRUCTION gains two new hard rules, phrased generically:
       - no material/texture adjective used as a class discriminator
       - no prompt that names only a thin sub-part/boundary of the class
    2. The per-class "max_prompts" field and its code-side truncation were removed
       to keep the same format as contexts_class_voc.json: prompt count is governed
       uniformly by the 2-4 target (rule 5) and, where needed, by a class's own
       "generation_notes" (rule 2), not by a separate numeric field.
    3. Defaults point at the Cityscapes config files.

Input : contexts_class_city.json  {class: {definition, anchor_prompts?, positive,
                                            negative, confusable_classes,
                                            generation_notes?}}
Output: adjust_prompt_city_v2.json   {class: [prompt1, prompt2, ...]}
        adjust_prompt_city_v2_meta.json  (raw response + config, for reproducibility)
...
class ClassPromptItem(BaseModel):
    ...
    prompts: list[str] = Field(
        description=(
            ...
            "only when one word loses essential meaning. Never rely on a "
            "material/texture adjective (wood, metal, brick, stone, concrete, etc.) "
            "or a thin sub-part/boundary term as the discriminator. No full "
            "sentences, no articles."
        )
    )
...
1. If the class context includes "anchor_prompts": these terms MUST be copied \
   ...
   for many classes the set of real-world visual sub-types is too large to enumerate, \
   so replacing the anchor with a small enumerated list causes recall loss on every \
   instance outside that list.
...
2. If "generation_notes" is present for a class, treat it as a HARD CONSTRAINT that \
   ...
   in rule 5). Read it carefully -- it documents a specific failure mode already \
   observed for that class.

3. NEVER use a raw material or texture adjective (wood, wooden, metal, metallic, \
   brick, stone, concrete, plastic, fabric, leather, etc.) as a class-decomposition \
   strategy, even if the class's own defining noun is present in the phrase (e.g. \
   "brick wall", "wooden fence", "metal fence"). Open-vocabulary text encoders are \
   strongly texture-conditioned: a material-specific phrase biases the predicted mask \
   toward that exact visual texture and causes recall loss on real instances made of a \
   different material. The only exception is when "definition" explicitly restricts \
   the class to a single material. Prefer a structural, functional, or positional \
   discriminator instead (e.g. "freestanding wall", "chain-link fence").

4. NEVER use a prompt that names only a thin boundary/sub-part of the class's full \
   area as a standalone prompt (e.g. "curb" for "sidewalk"). Matching this term \
   causes the predicted mask to cover only that thin sub-region instead of the full \
   class extent, lowering IoU. A boundary feature mentioned in "positive" may inform \
   your wording but must not become a prompt by itself unless it IS the entire class.

5. Unless "generation_notes" says otherwise, produce 2-4 total prompts per class \
...
6. STRONGLY prefer a single noun (e.g. "grass", "lawn", "sand"). Only use a 2-word \
...
7. Simplify/shorten entries from the given "positive" list -- do NOT invent concepts \
...
8. NEVER generate a prompt that duplicates or is synonymous with any entry in that \
   same class's own "negative" list.

9. Avoid prompts that could be easily confused with the listed "confusable_classes" -- \
   prefer words with high discriminative power against those classes.

10. Avoid generic, broad descriptors that pair a common non-material adjective (size, \
    upholstery, generic shape) with a generic noun (e.g. "large table", "upholstered \
    furniture") unless the class's own defining noun is present in the phrase -- these \
    tend to match many unrelated objects and cause over-segmentation. (Material/texture \
    adjectives are already fully excluded by rule 3, regardless of this exception.)

11. Output must be standalone nouns/noun phrases, not full sentences, no articles \
...
def build_user_content(class_contexts: dict) -> str:
    """Serialize contexts_class.json into the request content. Keep the original
...
def validate_and_clean(
    ...
    - Enforce "anchor_prompts": if the LLM dropped one, auto-prepend it rather than
      just warning. This is the exact failure mode observed in practice (the bare
      class term silently replaced by a narrower synonym), so it is corrected
      automatically instead of relying on a manual pass.
    """
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate adjust_prompt_city_v2.json from contexts_class_city.json via Gemini."
    )
    parser.add_argument("--input", default=str(Path(__file__).resolve().parent.parent / "configs" / "contexts_class_city.json"))
    parser.add_argument("--output", default=str(Path(__file__).resolve().parent.parent / "configs" / "adjust_prompt_city.json"))
