...
pc59_common.py
==============
Shared library for every step of the PC59 pipeline.

Section A (DATASET SPEC) is the ONLY place that knows anything specific to PASCAL-Context 59:
...
run_pipeline_pc59.py can import this module for its preflight/plan without a GPU stack.
"""
...
# ============================================================================
# A. DATASET SPEC -- PASCAL-Context 59
# ============================================================================
DATASET = "pc59"
DATASET_TITLE = "PASCAL-Context 59"

ROOT = Path(__file__).resolve().parent
DATA_DIR = ROOT / "data"
IMAGES_DIR = DATA_DIR / "JPEGImages"                 # VOC2010 images, <id>.jpg
GT_DIR = DATA_DIR / "SegmentationClassContext"       # raw PNG: 0 = background/other, 1..59 = class
TRAIN_LIST = DATA_DIR / "pascal_context_train.txt"   # 4998 lines
VAL_LIST = DATA_DIR / "pascal_context_val.txt"       # 5105 lines
REQUIRED_DATA_PATHS = [IMAGES_DIR, GT_DIR, TRAIN_LIST, VAL_LIST]

CONFIG_DIR = ROOT / "configs"
TARGET_CLASSES_JSON = CONFIG_DIR / "target_classes.json"
ADJUST_PROMPT_JSON = CONFIG_DIR / "adjust_prompt_pc59.json"

# ABSOLUTE RULE: alphabetical order, hardcoded. NEVER derive it from 59_labels.txt (that file lists
# the 20 VOC classes first) -- SegmentationClassContext/ was built with an alphabetical LUT.
...
CLASSES = list(_PC59_CLASSES_ALPHABETICAL)
assert len(CLASSES) == 59

# Evaluation label space (baseline / val, "canonical"): 0..58 = class, 255 = ignore.
# Pixels with no confident SAM3 detection stay 255 -- NOT 0, which is "aeroplane".
EVAL_NUM_CLASSES = 59
EVAL_CLASS_OFFSET = 0                      # eval index of CLASSES[i] is i + EVAL_CLASS_OFFSET
EVAL_NO_PREDICTION = 255
EVAL_IGNORE = 255
EVAL_METRIC_INDICES = list(range(59))      # classes reported and averaged into mIoU
EVAL_EXTRA_NAMES = {}                      # names for eval indices that are not in CLASSES

# Training label space (get_coarse / train): RAW pixel value, i+1 for the i-th class, 0 = background.
TRAIN_RAW_VALUE = {name: i + 1 for i, name in enumerate(CLASSES)}
...
TRAIN_PARAMS = {
    ...
    "patience": 8,
    "focal_gamma": 1.5,
    # Neutral for every target class -- identical to the PC59 no-LLM run (keep the two arms comparable).
...
def _read_id_list(path):
    """'JPEGImages/<id>.jpg GroundTruth_trainval_png/<id>.png' per line -> bare ids."""
...
def list_ids(subset):
    if subset == "train":
        return _read_id_list(TRAIN_LIST)
    if subset == "val":
        return _read_id_list(VAL_LIST)
...
def gt_path(img_id, subset):
    p = GT_DIR / f"{img_id}.png"
    return p if p.exists() else None


def cache_key(img_id):
    """File stem used for every per-image cache (coarse, DINOv2, val)."""
    return img_id
...
def load_gt_train_raw(img_id, subset):
    """Raw mask for get_coarse/train: 0 = background/other, 1..59 = class. Not normalized."""
    return np.array(Image.open(gt_path(img_id, subset)), dtype=np.uint8)


def train_valid_mask(raw):
    """PC59 raw masks have no void value -- every pixel is valid (background = 'other')."""
    return np.ones(raw.shape, dtype=np.uint8)


def load_gt_eval(img_id, subset):
    """Raw -> canonical: raw - 1, negative (raw 0 = background) -> 255 (ignore)."""
    raw = np.array(Image.open(gt_path(img_id, subset)), dtype=np.uint8)
    out = raw.astype(np.int16) - 1
    out[out < 0] = EVAL_IGNORE
